...
if IDX{ix} ==0;
   IM(:,:,ix) = zeros(ROI_Data{ix}.Ysiz_DS(1),ROI_Data{ix}.Ysiz_DS(2));
else
...
plot3(A.out.Location2(:,1),A.out.Location2(:,2),A.out.Location2(:,3),'Color',[0.3 0.1 0.1]);% plot the flight trajectory in space
plot3(B.out.Location2(:,1),B.out.Location2(:,2),B.out.Location2(:,3),'Color',[0.1 0.3 0.1]);% plot the flight trajectory in space
plot3(C.out.Location2(:,1),C.out.Location2(:,2),C.out.Location2(:,3),'Color',[0.1 0.1 0.3]);% plot the flight trajectory in space
...
Spike_times = ROI_Data{1, iii}.Alignment.out.video_times(xy)-offset; % convert this to 'spike time'
