...
for d = 1:length(dirTop)
    ...
    snakeTrace(d) = load(sp(end).name);
    fp = dir('*flightPaths.mat');
    flightPaths(d) = load(fp(end).name);
    close all;
    cd(dirTop(d).folder)
end
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    for flight_i = 1:length(flightPaths(day_i).flightPaths.clusterIndex{2})
        subplot(3,length(ROIs_gal(:,1)),day_i);
        plot3(flightPaths(day_i).flightPaths.pos(1,:,flightPaths(day_i).flightPaths.clusterIndex{2}(flight_i)),flightPaths(day_i).flightPaths.pos(2,:,flightPaths(day_i).flightPaths.clusterIndex{2}(flight_i)),flightPaths(day_i).flightPaths.pos(3,:,flightPaths(day_i).flightPaths.clusterIndex{2}(flight_i)),...
        ...
        plot(1:length(snakeTrace(day_i).snakeTrace.smoothSpeedRawFlight{2}(flight_i,1:xlimFlight)),snakeTrace(day_i).snakeTrace.smoothSpeedRawFlight{2}(flight_i,1:xlimFlight));
        hold on;
        if day_i == 1
            ylabel('Velocity (m/s)');
            xlabel('Time (s)');
    ...
    imagesc(snakeTrace(day_i).snakeTrace.normMeanTraceEachFlight{2}(:,1:xlimCalcium),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normTraceFlight{2}(snakeTrace(day_i).snakeTrace.IFlight{2},1:xlimCalcium),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    %plot all ROIs according to preferred sorting individually
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normTraceFlight{2}(snakeTrace(1).snakeTrace.IFlight{2},1:xlimCalcium),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    for flight_i = 1:length(flightPaths(day_i).flightPaths.clusterIndex{2})
        ...
        %plot velocity for cluster #2
        subplot(3,length(ROIs_gal(:,1)), length(ROIs_gal(:,1)) + day_i);
        plot(1:length(snakeTrace(day_i).snakeTrace.smoothSpeedRawPre{2}(flight_i,1:xlimFlightPre)),snakeTrace(day_i).snakeTrace.smoothSpeedRawPre{2}(flight_i,1:xlimFlightPre));
        hold on;
        if day_i == 1
            ylabel('Velocity (m/s)');
            xlabel('Time (s)');
    ...
    %across all days
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normMeanTraceEachPre{2}(:,1:xlimCalciumPre),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    %plot all ROIs according to preferred sorting individually
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normTracePre{2}(snakeTrace(day_i).snakeTrace.IPre{2},1:xlimCalciumPre),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    %plot all ROIs according to preferred sorting individually
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normTracePre{2}(snakeTrace(1).snakeTrace.IPre{2},1:xlimCalciumPre),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    for flight_i = 1:length(flightPaths(day_i).flightPaths.clusterIndex{2})
        ...
        %plot velocity for cluster #2
        subplot(3,length(ROIs_gal(:,1)), length(ROIs_gal(:,1)) + day_i);
        plot(1:length(snakeTrace(day_i).snakeTrace.smoothSpeedRawPost{2}(flight_i,1:xlimFlightPost)),snakeTrace(day_i).snakeTrace.smoothSpeedRawPost{2}(flight_i,1:xlimFlightPost));
        hold on;
        if day_i == 1
            ylabel('Velocity (m/s)');
            xlabel('Time (s)');
    ...
    %across all days
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normMeanTraceEachPost{2}(:,1:xlimCalciumPost),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    %plot all ROIs according to preferred sorting individually
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normTracePost{2}(snakeTrace(day_i).snakeTrace.IPost{2},1:xlimCalciumPost),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    %plot all ROIs according to preferred sorting individually
    subplot(3,length(ROIs_gal(:,1)), 2*length(ROIs_gal(:,1)) + day_i);
    imagesc(snakeTrace(day_i).snakeTrace.normTracePost{2}(snakeTrace(1).snakeTrace.IPost{2},1:xlimCalciumPost),[2 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
...
for day_i = 1:length(ROIs_gal(:,1))
    ...
    for flight_i = 1:2:length(flightPaths(day_i).flightPaths.clusterIndex{2})
    ...
    imagesc(snakeTrace(day_i).snakeTrace.normTraceOdd{2}(:,1:xlimCalcium),[1.5 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
    ...
    for flight_i = 2:2:length(flightPaths(day_i).flightPaths.clusterIndex{2})
    ...
    imagesc(snakeTrace(day_i).snakeTrace.normTraceEven{2}(:,1:xlimCalcium),[1.5 4.5]);
    colormap(hot);
    if day_i == 1
        ylabel('ROI #');
        xlabel('Time (s)');
