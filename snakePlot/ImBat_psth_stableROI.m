function [psth_stableROI] = ImBat_psth_stableROI
...
for day_i = 1:length(dirTop)
    ...
    for clust_ii = 1:nClusts
        nFlights = size(flightPaths.clusterIndex{clust_ii},1);
        traceMean{day_i}{clust_ii} = zeros(nROIs,length(snakeTrace_cRaw.normTraceFlight{clust_ii}(1,:)));
        traceStd{day_i}{clust_ii} = zeros(nROIs,length(snakeTrace_cRaw.sdTraceFlight{clust_ii}(1,:)));
        ...
        for ROI_i = 1:nROIs
            try
                traceMean{day_i}{clust_ii}(ROI_i,:) = snakeTrace_cRaw.normTraceFlight{clust_ii}(ROIs_gal(day_i,ROI_i),:);
                traceStd{day_i}{clust_ii}(ROI_i,:) = snakeTrace_cRaw.sdTraceFlight{clust_ii}(ROIs_gal(day_i,ROI_i),:);
            catch
                traceMean{day_i}{clust_ii}(ROI_i,:) = zeros(1,length(snakeTrace_cRaw.normTraceFlight{clust_ii}(1,:)));
                traceStd{day_i}{clust_ii}(ROI_i,:) = zeros(1,length(snakeTrace_cRaw.sdTraceFlight{clust_ii}(1,:)));
            end
            velocity{day_i}{clust_ii}{ROI_i} = zeros(nFlights,length(snakeTrace_cRaw.smoothSpeedRawFlight{clust_ii}(1,:)));
            ...
            traceIndiv{day_i}{clust_ii}{ROI_i} = zeros(nFlights,length(snakeTrace_cRaw.sdTraceFlight{clust_ii}(1,:)));
            for flight_i = 1:nFlights
                velocity{day_i}{clust_ii}{ROI_i}(flight_i,:) = snakeTrace_cRaw.smoothSpeedRawFlight{clust_ii}(flight_i,:);
                flight{day_i}{clust_ii}{ROI_i}(flight_i,:,:,:) = flightPaths.pos(1:3,:,flightPaths.clusterIndex{clust_ii}(flight_i));
                try
                    traceIndiv{day_i}{clust_ii}{ROI_i}(flight_i,:) = snakeTrace_cRaw.normTraceRawFlight{clust_ii}(flight_i,:,ROIs_gal(day_i,ROI_i));
                catch
                    traceIndiv{day_i}{clust_ii}{ROI_i}(flight_i,:) = zeros(1,length(snakeTrace_cRaw.normTraceRawFlight{clust_ii}(flight_i,:,ROIs_gal(day_i,1))));
                end
            end
        end
    end
    ...
    if plotPSTHclustFlag == 1
        ...
        nCells = size(ROIs_gal,2); %number of cells in this day
        nClusts = size(snakeTrace_cRaw.smoothTraceRawFlight,2); %number of clusters in this day
            ...
            try
                for clust_i = 1:nClusts
                    ...
                    for flight_i = 1:nFlights
                        ...
                        plot(1:length(snakeTrace_cRaw.smoothSpeedRawFlight{clust_i}(flight_i,:)),snakeTrace_cRaw.smoothSpeedRawFlight{clust_i}(flight_i,:),'Color',colDay(clust_i,:));
                        hold on;
                        if clust_i == 1
                            ylabel('Velocity (m/s)');
                            yt = get(gca,'YTick');
                        ...
                        plot(1:length(snakeTrace_cRaw.normTraceRawFlight{clust_i}(flight_i,:,ROIs_gal(day_i,cell_i))),(snakeTrace_cRaw.normTraceRawFlight{clust_i}(flight_i,:,ROIs_gal(day_i,cell_i)))+flight_i*6,'Color',colDay(clust_i,:));
                        hold on;
                        if clust_i == 1
                        ...
                        xlim([0 length(snakeTrace_cRaw.normTraceRawFlight{clust_i}(flight_i,:,ROIs_gal(day_i,cell_i)))]);
                    end
                    ...
                    set(gca,'xticklabel',[]);
                    xlim([0 length(snakeTrace_cRaw.normTraceFlight{clust_i}(ROIs_gal(day_i,cell_i),:))]);
                end
            catch
...
% % for cell_ii = 1:length(ROIs_gal(1,:))
