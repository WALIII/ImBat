...
for clust_i = 1:nClusters %length(flightPaths.clusterIndex)
     ...
     %calculate max duration for each cluster of trajectories
        ...
        %meanTrace{clust_i}=zeros(length(cellData.results.C_raw(:,1)),maxDur+preWindow+1);
    %initialize the vector to store the neural activity of each flight
        traceFlight{clust_i} = zeros(length(flightPaths.clusterIndex{clust_i}),maxDur+prePadCalcium+postPadCalcium+1,length(cellData.results.C_raw(:,1)));
        speedFlight{clust_i} = zeros(length(flightPaths.clusterIndex{clust_i}),maxDurSpeed+prePadSpeed+postPadSpeed+1);
        tracePre{clust_i} = zeros(length(flightPaths.clusterIndex{clust_i}),preFlightPadCalcium+1,length(cellData.results.C_raw(:,1)));
        speedPre{clust_i} = zeros(length(flightPaths.clusterIndex{clust_i}),preFlightPadSpeed+1);
        tracePost{clust_i} = zeros(length(flightPaths.clusterIndex{clust_i}),postFlightPadCalcium+1,length(cellData.results.C_raw(:,1)));
        ...
        %for each cell
    for cell_i = 1:length(cellData.results.C_raw(:,1))
        ...
        for trace_i = 1:length(flightPaths.clusterIndex{clust_i})
            try
                ...
                smoothTraceRawPreFlightPost{clust_i}(trace_i,:,cell_i) = smooth(tracePreFlightPost{clust_i}(trace_i,:,cell_i),10);
                ...
                smoothSpeedRawPreFlightPost{clust_i}(trace_i,:) = smooth(speedPreFlightPost{clust_i}(trace_i,:),100);
                
            catch
                disp('catch')
                %sizeToRecordingEnd = size(cellData.results.C_raw(cell_i,closestIndexStart(trace_i) - prePadCalcium:end),2);
                %sizeToTraceEnd = size(traceFlight{clust_i}(trace_i,:),2);
                %traceFlight{clust_i}(trace_i,:,cell_i) = (cellData.results.C_raw(cell_i,closestIndexStart(trace_i) - prePadCalcium:end + postPadCalcium)+(zeros(1,sizeToTraceEnd - sizeToRecordingEnd)));
...
for cell_ii = 1:length(cellData.results.C_raw(:,1))
...
for clust_iii = 1:nClusters
    ...
    for cell_iii = 1:length(cellData.results.C_raw(:,1))
...
snakeTrace.preFlightPadCalcium = preFlightPadCalcium;
snakeTrace.postFlightPadCalcium = postFlightPadCalcium;
snakeTrace.preFlightPadSpeed = preFlightPadSpeed;
snakeTrace.postFlightPadSpeed = postFlightPadSpeed;
...
snakeTrace.traceFlightIdx = traceFlightIdx;
snakeTrace.normTraceRawPre = normTraceRawPre;
snakeTrace.normTraceRawFlight = normTraceRawFlight;
snakeTrace.normTraceRawPost = normTraceRawPost;
snakeTrace.traceFlight = traceFlight;
snakeTrace.tracePreFlightPost = tracePreFlightPost; 
snakeTrace.meanSpeedPreFlightPost = meanSpeedPreFlightPost;
snakeTrace.meanTracePreFlightPost = meanTracePreFlightPost;
snakeTrace.speedPreFlightPost = speedPreFlightPost; 
snakeTrace.smoothTraceRawPreFlightPost = smoothTraceRawPreFlightPost;
snakeTrace.normTraceRawPreFlightPost = normTraceRawPreFlightPost;
snakeTrace.normTraceRawPreFlightPost = normTraceRawPreFlightPost;
snakeTrace.smoothSpeedRawPreFlightPost = smoothSpeedRawPreFlightPost;
