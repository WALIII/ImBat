function [h,p] = ImBat_MCS_FlightConditioning_Physically_Possible_v_Actual(c_s_34,pruned_dataset,takeoff_locations_cpu,outliers,varargin)
...
for i=1:lk
    for k=1:lk
        for m=3:size(DS,1)   
            if DS(m-1) == i & DS(m-2) == k & ~ismember(m,has_incorrect_previous_flight)
                for j=1:size(unique(DS),1)
                    if DS(m) == j
                        B((i-1)*lk+k,j) = B((i-1)*lk+k,j) + 1;
                    end
                end
            elseif DS(m-1) == i & DS(m-2) == k & ismember(m,has_incorrect_previous_flight)
                tt = tt+1;
                for j=1:size(unique(DS),1)
