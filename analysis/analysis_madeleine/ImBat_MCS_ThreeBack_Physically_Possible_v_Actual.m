function [h,p] = ImBat_MCS_ThreeBack_Physically_Possible_v_Actual(c_s_34,pruned_dataset,takeoff_locations_cpu,outliers,varargin)
...
%% D Now construct DD, conditioning also on the previous flight and previous takeoff location and previous previous flight
% D = P(F(t+1) | T(t), F(t), T(t-1), F(t-1))
clear D;
lyy = lk;

D = zeros(li*lk*lxx*lk,lk);
tt=0;
for i=1:li % Takeoff pos
    for k=1:lk                                      % Prev flight type 
        for xx = 1:lxx % Prev Takeoff pos
            for yy = 1:lyy
                disp(((i-1)*lk*lxx*lyy) + ((k-1)*lxx*lyy) + (xx-1)*lyy + yy)
                for m=3:size(DS,1)
                    if takeoff_position(m) == i & DS(m-1) == k & takeoff_position(m-1) == xx & DS(m-2) == yy & ~ismember(m,has_incorrect_previous_flight)
                        for j=1:lk
                            if DS(m) == j
                                D(((i-1)*lk*lxx*lyy) + (k-1)*lxx*lyy + (xx-1)*lyy + yy, j) = D(((i-1)*lk*lxx*lyy) + (k-1)*lxx*lyy + (xx-1)*lyy + yy, j) + 1;
                            end
                        end
                    elseif takeoff_position(m) == i & DS(m-1) == k & takeoff_position(m-1) == xx & DS(m-2) == yy & ismember(m,has_incorrect_previous_flight)
                        tt = tt+1;
                        for j=1:lk
                            if c_s_34(outliers(tt)) == j
                                D(((i-1)*lk*lxx*lyy) + (k-1)*lxx*lyy + (xx-1)*lyy + yy, j) = D(((i-1)*lk*lxx*lyy) + (k-1)*lxx*lyy + (xx-1)*lyy + yy, j) + 1;
...
% Calculate the conditional probability; each row sums to 1
DD = zeros(size(D,1),size(D,2));
for i=1:size(D,1)
    row_of_concern = D(i,:);
    for j=1:size(row_of_concern,2)
        DD(i,j) = D(i,j)/sum(row_of_concern);
    end
end
...
clear TPPC;
% Calculate the prior distribution of previous flight type and positions 
TPPC = zeros(li*lk*lxx*lyy,1);
for i=1:li
    for k=1:lk
        for xx = 1:lxx
            for yy = 1:lyy
                for m=3:size(DS,1)
                    if takeoff_position(m) == i & DS(m-1) == k & takeoff_position(m-1) == xx & DS(m-2) == yy
                        TPPC(((i-1)*lk*lxx*lyy) + (k-1)*lxx*lyy + (xx-1)*lyy + yy) = TPPC(((i-1)*lk*lxx*lyy) + (k-1)*lxx*lyy + (xx-1)*lyy + yy) + 1;
                    end
                end
            end
        end
...
Takeoff_Position_Cond_Prob_D = TPPC./sum(TPPC);
...
% DD
Joint_DD = zeros(size(DD,1),size(DD,2));
for i=1:size(Takeoff_Position_Cond_Prob_D,1)
    t_p_i = Takeoff_Position_Cond_Prob_D(i);
    DD_row = DD(i,:);
    Joint_DD(i,:) = DD_row'*t_p_i;
end

Joint_DD_vec = reshape(Joint_DD,1,size(Joint_DD,1)*size(Joint_DD,2));

%% KS Test
clear p h;
[h,p] = kstest2(Joint_BB_vec,Joint_DD_vec); 
