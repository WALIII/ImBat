function [output] = ImBat_PlaceCells(ROI_Data,varargin);
% ImBat_PlaceCells
...
ROI2Plot = 1;%1:size(neuron.C_raw,1);
day2use = 1;
out = ROI_Data{day2use}.Alignment.out;
neuron = ROI_Data{day2use}.ROIs.results;
shade2use = 0.25;
Spk_Thresh = 0.025;
az = -38;
el = 17;
...
offset = -.100; % account for slow calcium estimation ~move locations back 100ms in time... This is the knob to turn for 'prospective' coding...

nparams=length(varargin);

% Flight Pre-processing- Remove that pesky first flight that starts at 0:0;
...
for i=1:2:nparams
    switch lower(varargin{i})
        ...
        case 'flights'
            ...
            % find NaNs in out.Flights
            a2 = isnan(out.flights(1:end,1));
            out.Location2(a2,:) = NaN;
            out.flights = out.Location2;
        case 'save'
...
for ii = ROI2Plot; % for each cell
    ...
    try % this 'try/catch' is to avoid crashing if cells are not active in plotting window...
        ...
        scatter3(LX,LY,LZ,(PH*200)+1,'MarkerFaceColor','r','MarkerEdgeColor','r',...
