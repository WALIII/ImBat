...
% User params
min_flight_number = 40; %40 % min number of flights needed
min_prom = 0.25; % SD * 10 % disp(min_prom);
only_max_peak = 0; % only take the max- otherwise,  NOTE: there may be more than one peak per ROI!
...
for ii = 1:size(FlightAlignedROI_combined{1}.C,1) % first 10 cells:
    ...
    % only use peaks in the flight pad ( i.e. exclude reward...)
    flightInterval = mean(FlightAlignedROI_combined{1}.FlightLength);
    stTime = 550; % 250
    stpTime = stTime+flightInterval; % 1200
        ...
        % If you only want to take the max peak:
    if only_max_peak ==1;
        [temp1 temp2] = max(Bpks);
        Blocs = Blocs(temp2);
        clear temp1 temp2
    end
    
    
    numPeaks(ii) = size(Blocs,1); % save the number of peaks for later. 
    ...
    if isempty(Blocs)
        ...
        for iii = 1: size(Blocs,1);
            ...
            meanPoint(1) = mean(flPeaksX(idx2use));
            meanPoint(2) = mean(flPeaksY(idx2use));
            meanPoint(3) = mean(flPeaksZ(idx2use));
            ...
            out2.calROI_ID{counter} = ii; % ROI_ID- since possibly there is more than one peak per ROI
            counter = counter+1;
        end
    end
...
%% Plotting: (NOTEL will also work outside of function)
try
    figure();
    counter = 1;
