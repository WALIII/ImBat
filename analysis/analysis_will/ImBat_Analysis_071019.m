...
if exist('Y') ==0; % load in Y from local directory
    ...
    try
        ...
        load([ROI_Data{day}.date,'/extracted/',ROI_Data{day}.folder,'/Motion_corrected_Data_DS.mat'])
    end
end
...
figure();
for i = 1:numClust
    [im1_rgb norm_max_proj{i},I{i},idx_img{i}] = CABMI_allpxs(imresize(Ydata3{i}(:,:,100:200),3),'filt_rad',filt2use,'exp',exp2use);
end
