...
%% Concat data
for ii = 1:size(out,2);
   out_final.x_2save = cat(1,out_final.x_2save,out{ii}.x_2save);
   out_final.y_2save = cat(1,out_final.y_2save,out{ii}.x_2save);
   out_final.err_2save = cat(1,out_final.err_2save,out{ii}.err_2save);
   out_final.FL = cat(1,out_final.FL,out{ii}.FL);
end
        out_final.ROI_ON = out{1}.ROI_ON;
        
        
% resort based on max location:
[~, sort_idx] = sort(out_final.x_2save(:,1));
...
out_final2 = out_final;
offset2use = 120;
out_final2.ROI_ON = out_final2.ROI_ON+offset2use;;
out_final2.x_2save = out_final2.x_2save-out_final2.FL+offset2use-20;
[~, sort_idx] = sort(out_final2.x_2save(:,1),'descend');
out_final2.x_2save = out_final2.x_2save(sort_idx,:);
out_final2.y_2save = out_final2.y_2save (sort_idx,:);
out_final2.err_2save = out_final2.err_2save(sort_idx,:);
out_final2.FL = out_final2.FL(sort_idx,:);
...
out_final3.y_2save(out_final3.x_2save(:,1)-out_final3.ROI_ON<-30,:) = [];
out_final3.err_2save(out_final3.x_2save(:,1)-out_final3.ROI_ON<-30,:) = [];
out_final3.FL(out_final3.x_2save(:,1)-out_final3.ROI_ON<-30,:) = [];
out_final3.x_2save(out_final3.x_2save(:,1)-out_final3.ROI_ON<-30,:) = [];

rn2rm = out_final3.x_2save(:,1)-out_final3.ROI_ON-(out_final3.FL+60)>0;
out_final3.y_2save(rn2rm,:) = [];
out_final3.err_2save(rn2rm,:) = [];
out_final3.x_2save(rn2rm,:) = [];
out_final3.FL(rn2rm,:) = [];
