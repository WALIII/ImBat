function [metadata] = CNMFe_extract2(varargin)
...
metadata.cnmfe.min_pnr = 30;                  % minimum peak-to-noise ratio for a seeding pixel (low values are discouraged)
metadata.cnmfe.min_pixel = metadata.cnmfe.gSig^2;             % minimum number of nonzero pixels for each neuron (was gSig^2)
bd = 4;                         % number of rows/columns to be ignored in the boundary (mainly for motion corrected data)
...
saveastiff(uint16(PNR),'PNR.tif',options);
saveastiff(uint8(Cn*255),'Cn.tif',options);
