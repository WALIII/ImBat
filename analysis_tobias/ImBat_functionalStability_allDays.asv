...
days = [day1:dayEnd]; %all days concat
...
meanMax_act_aligned = zeros(size(act_aligned,1),size(act_aligned,2));
stdMax_act_aligned = zeros(size(act_aligned,1),size(act_aligned,2));
meanMax_act_pre = zeros(size(act_aligned,1),size(act_aligned,2));
stdMax_act_pre = zeros(size(act_aligned,1),size(act_aligned,2));
meanMax_act_dur = zeros(size(act_aligned,1),size(act_aligned,2));
stdMax_act_dur = zeros(size(act_aligned,1),size(act_aligned,2));
meanMax_act_post = zeros(size(act_aligned,1),size(act_aligned,2));
stdMax_act_post = zeros(size(act_aligned,1),size(act_aligned,2));
ylab = cell(1,3);
overlapSorted = cell(size(act_aligned,1),3);
intersectDays = cell(1,3);

%determine the max,mean, and std for the pre, dur, post data
for day_i = 1:size(act_aligned,1)
    for roi_i = 1:size(act_aligned,2)
        for flight_i = 1:size(act_aligned{day_i,roi_i},1)
            [~,max_act_aligned{day_i,roi_i}(flight_i,1)] = max(act_aligned{day_i,roi_i}(flight_i,:));
            [~,max_act_pre{day_i,roi_i}(flight_i,1)] = max(act_pre{day_i,roi_i}(flight_i,:));
            [~,max_act_dur{day_i,roi_i}(flight_i,1)] = max(act_dur{day_i,roi_i}(flight_i,:));
            [~,max_act_post{day_i,roi_i}(flight_i,1)] = max(act_post{day_i,roi_i}(flight_i,:));
        end
    ...
    for phase_i = 1:size(selectiveCells,2)
        ...
        if phase_i == 1
            [BSelectiveCells{day_i,phase_i},ISelectiveCells{day_i,phase_i}] = sort(meanMax_act_pre(days(day_i),findSelectiveCells{day_i,phase_i}));
        elseif phase_i == 2
            [BSelectiveCells{day_i,phase_i},ISelectiveCells{day_i,phase_i}] = sort(meanMax_act_dur(days(day_i),findSelectiveCells{day_i,phase_i}));
        elseif phase_i == 3
            [BSelectiveCells{day_i,phase_i},ISelectiveCells{day_i,phase_i}] = sort(meanMax_act_post(days(day_i),findSelectiveCells{day_i,phase_i}));
        end
    end
end
...
sgtitle('Functional Stability of Consistently Selectively Active Cells: All Days: Cluster 2');
colorDays = jet(nDays); %make the color vector for each day
for day_i = 1:size(selectiveCells,1)
    ...
    for phase_i = 1:size(selectiveCells,2)
        for roi_i = 1:length(overlapSorted{day_i,phase_i})
            ...
            if phase_i == 1
                p1(day_i) = plot(BSelectiveCells{day_i,phase_i}(intersectDays{phase_i}(roi_i)),roiStableCounter,'.','Color',colorDays(day_i,:),'MarkerSize',20);
                p1(day_i).Color(4) = 0.5;
                if stdFlag ==1
                    p2 = plot(BSelectiveCells{day_i,phase_i}(intersectDays{phase_i}(roi_i))-round(stdMax_act_dur(day_i,roi_i)/2):1:BSelectiveCells{day_i,phase_i}(overlapSorted{1,phase_i}(roi_i))+round(stdMax_act_dur(day_i,roi_i)/2),...
                        ones(round(stdMax_act_dur(roi_i,phase_i)/2)*2)*roiStableCounter,'-','Color',colorDays(day_i,:),'LineWidth',1);
                end
            elseif phase_i == 2
                p1(day_i) = plot(preDur+BSelectiveCells{day_i,phase_i}(intersectDays{phase_i}(roi_i)),roiStableCounter,'.','Color',colorDays(day_i,:),'MarkerSize',20);
                p1(day_i).Color(4) = 0.5;
                if stdFlag ==1
                    p2 = plot(1+preDur+BSelectiveCells{day_i,phase_i}(intersectDays{phase_i}(roi_i))-round(stdMax_act_dur(day_i,roi_i)/2):1:preDur+BSelectiveCells{day_i,phase_i}(overlapSorted{1,phase_i}(roi_i))+round(stdMax_act_dur(day_i,roi_i)/2),...
                        ones(round(stdMax_act_dur(day_i,roi_i)/2)*2)*roiStableCounter,'-','Color',colorDays(day_i,:),'LineWidth',1);
                end
            elseif phase_i == 3
                ...
                p1(day_i).Color(4) = 0.5;
                if stdFlag ==1
                    p2 = plot(1+preDur+flightLength+BSelectiveCells{day_i,phase_i}(intersectDays{phase_i}(roi_i))-round(stdMax_act_dur(day_i,roi_i)/2):1:preDur+flightLength+BSelectiveCells{day_i,phase_i}(intersectDays{phase_i}(roi_i))+round(stdMax_act_dur(day_i,roi_i)/2),...
...
if saveFlag == 1
    saveas(functionalStabilityDotPlot_consistent_all,[saveDir filesep 'Gal_200311and20_functionalStability_dotPlot_selectiveCells_clust2_consistent_allDays_std_offset' datestr(now,'yymmdd-HHMM') '.tif']);
    savefig(functionalStabilityDotPlot_consistent_all,[saveDir filesep 'Gal_200311and20_functionalStability_dotPlot_selectiveCells_clust2_consistent_allDays_std_offset' datestr(now,'yymmdd-HHMM') '.fig']);
...
drawnow; 
if saveFlag == 1
    saveas(functionalStabilityDotPlot_consistent_all,[saveDir filesep 'Gal_200311and20_functionalStability_dotPlot_selectiveCells_clust2_consistent_allDays_zoom_std_offset' datestr(now,'yymmdd-HHMM') '.tif']);
    savefig(functionalStabilityDotPlot_consistent_all,[saveDir filesep 'Gal_200311and20_functionalStability_dotPlot_selectiveCells_clust2_consistent_allDays_zoom_std_offset' datestr(now,'yymmdd-HHMM') '.fig']);
