...
%smoothAvgSpiking = zscore(smooth(mean((full(cellData.results.C_raw(1:topROILocal,:))),1),25));%-min(data(p).image_data.S(1:numCells,:));
    ...
    for i = 1:topROILocal %[9 31 16 27 26 29 41 46 54 61 68 98 86 87 151]%
        ...
        try
            plot(alignment.out.video_timesDS,(zscore(smoothdata(cellData.results.C_raw(i,:),'movmedian',3)))+i*6) %may have to tweak the +i*6 at the end
        catch
            ...
            plot(alignment.out.video_timesDS(1:end-1),(zscore(smoothdata(cellData.results.C_raw(i,:),'movmedian',3)))+i*6) %may have to tweak the +i*6 at the end
        end
    end 
