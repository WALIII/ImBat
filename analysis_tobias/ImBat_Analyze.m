...
%         end
        ...
        if plotROIFlag == 1
            ...
            [ROIoverlay] = ImBat_ROIoverlay(cellData.results,'centroid',centroidFlag,'binarymask',binaryMaskFlag,'roiheat',roiHeatFlag,'batname',batName,'datesesh',dateSesh,'sessiontype',sessionType);
    ...
    if plotPlaceCellsFlag == 1 && strcmp(extractBefore(sessionType,'-'),'fly')
       mkdir([imageFolders(kk).folder,'/',imageFolders(kk).name,'/analysis/placeCells']);
    ...
    if plotSnakesFlag == 1 && strcmp(extractBefore(sessionType,'-'),'fly')
        cd([imageFolders(kk).folder ,'/',imageFolders(kk).name,'/analysis'])
        mkdir('snakePlots')
        cd([imageFolders(kk).folder,'/',imageFolders(kk).name,'/analysis/snakePlots'])
        ...
        saveas(snakeTrace.snakePlot_clust, [imageFolders(kk).folder filesep imageFolders(kk).name filesep 'analysis' filesep 'snakePlots' filesep fileName '_snakePlots_clust.svg']);
