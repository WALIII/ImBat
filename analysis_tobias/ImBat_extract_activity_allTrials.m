...
for day_i = 1:length(nDays) %for each day
    ...
    %make gaussian filter based on dff function
    gSig = 1;
    gSiz = 4.5*gSig; 
    ...
    vidData.Y = imfilter(vidData.Y,psf,'symmetric');
    % Take median of movie
    Y_med = median(vidData.Y,3);
    Y_min = min(vidData.Y,3);
    Ydff = vidData.Y - Y_med;
    YmaxFull{day_i} = max(Ydff,[],3);
    YmaxFull{day_i} = imresize(YmaxFull{day_i},scaling);
    ...
    for clust_i = 1:length(act_pre)
        for roi_i = 1:length(nRois)
            ...
            frames_pre{clust_i}{day_i} = NaN(1,vidData.Ysiz(1),vidData.Ysiz(2),1500);
            frames_dur{clust_i}{day_i} = NaN(1,vidData.Ysiz(1),vidData.Ysiz(2),1500);
            frames_post{clust_i}{day_i} = NaN(1,vidData.Ysiz(1),vidData.Ysiz(2),1500);
        end
    end
    ...
    for clust_i = 1:nClusts
        ...
        frames_pre{clust_i}{day_i} = zeros(nFlights{clust_i},vidData.Ysiz(1),vidData.Ysiz(2),lenPre{clust_i});
        frames_dur{clust_i}{day_i} = zeros(nFlights{clust_i},vidData.Ysiz(1),vidData.Ysiz(2),lenFlight{clust_i});
        frames_post{clust_i}{day_i} = zeros(nFlights{clust_i},vidData.Ysiz(1),vidData.Ysiz(2),lenPost{clust_i});        
        %roiCounter = 1;r
        for roi_i = 1:length(nRois)
        ...
        postPad = 7 * Fs_cnmfe;
        for flight_i = 1:nFlights{clust_i}
            %build matrices with the imaging raw frames for flight-aligned
            %max projections
            frames_pre{clust_i}{day_i}(flight_i,:,:,:) = Ydff(:,:,sData.startIdxTrace{clust_i}(flight_i)-sData.preFlightPadCalcium:sData.startIdxTrace{clust_i}(flight_i));
            try
            frames_dur{clust_i}{day_i}(flight_i,:,:,:) = Ydff(:,:,sData.startIdxTrace{clust_i}(flight_i)-prePad:...
                sData.endIdxTrace{clust_i}(flight_i) + (sData.medDur{clust_i}-sData.dur{clust_i}(flight_i))+ postPad);
            frames_post{clust_i}{day_i}(flight_i,:,:,:) = Ydff(:,:,sData.endIdxTrace{clust_i}(flight_i):sData.endIdxTrace{clust_i}(flight_i)+sData.postFlightPadCalcium);
            catch
            frames_dur{clust_i}{day_i}(flight_i,:,:,1:length(Ydff(:,:,sData.startIdxTrace{clust_i}(flight_i)-prePad:end)))...
                = Ydff(:,:,sData.startIdxTrace{clust_i}(flight_i)-prePad:end);
            frames_post{clust_i}{day_i}(flight_i,:,:,1:length(Ydff(:,:,sData.endIdxTrace{clust_i}(flight_i):end)))...
        ...
        meanFrames_pre{clust_i}{day_i} = squeeze(nanmean(frames_pre{clust_i}{day_i},1));
        meanFrames_dur{clust_i}{day_i} = squeeze(nanmean(frames_dur{clust_i}{day_i},1));
        meanFrames_post{clust_i}{day_i} = squeeze(nanmean(frames_post{clust_i}{day_i},1));
