...
batSpeedFilt = medfilt1(batSpeed,20);
...
for i = 1:length(LT)
    ...
    flightToFeederStart(i) = find(batSpeedFilt(1:flightToFeederEnd(i))==0,1,'last'); %minus the preflight # of seconds flightToFeederEnd(i) - preFlightTime
    ...
    fFeedEndxyz(i,1) = round(nanmean(mxFull(flightToFeederEnd(i):flightToFeederEnd(i)+trackData.VideoFrameRate/4)));
    fFeedEndxyz(i,2) = round(nanmean(myFull(flightToFeederEnd(i):flightToFeederEnd(i)+trackData.VideoFrameRate/4)));
    fFeedEndxyz(i,3) = round(nanmean(mzFull(flightToFeederEnd(i):flightToFeederEnd(i)+trackData.VideoFrameRate/4)));
...
for i = 1:length(LT)
    ...
    plot3(mxFull(flightFromFeederStart(i):flightFromFeederEnd(i)),myFull(flightFromFeederStart(i):flightFromFeederEnd(i)),mzFull(flightFromFeederStart(i):flightFromFeederEnd(i)),'LineWidth',2,'color',CM(i,:))
    hold on
    %pause(0.1)
end
...
for traj = 1 : 10
    ...
    for nf = allflights{ssf(traj)}
        plot3(mxFull(flightToFeederStart(nf):flightToFeederEnd(nf)),myFull(flightToFeederStart(nf):flightToFeederEnd(nf)),mzFull(flightToFeederStart(nf):flightToFeederEnd(nf)),'LineWidth',1,'Color',jj(traj*10,:))
        hold on
