    ...
    try
        if traj<ntrajectories + 1 % Only plot the first ntrajectories...
            ...
            for nf = allflights{ssf(traj)}
                ...
                plot3(mxFull(flight_starts(nf):flight_ends(nf)),myFull(flight_starts(nf):flight_ends(nf)),mzFull(flight_starts(nf):flight_ends(nf)),'LineWidth',1,'Color',jj(traj*180,:))
                hold on
